...
- **CI integration** — `.github/workflows/quality.yml` is the reusable workflow boundary; it resolves the manifest's immutable release, pins every Action to a full commit SHA, and passes explicit pull-request range refs to the same CLI gate.
...
- **Local and CI checking** — `quality-gate check` → `quality_gate/snapshot.py` → `quality_gate/runner.py` → validated manifest → structured verdict → Git hygiene, Gitleaks, workflow/documentation checks, Ruff, mypy, pytest, and report-only coverage when pinned. `.github/workflows/quality.yml` syncs the manifest release and passes pull-request base/head SHAs; `quality-gate audit` adds full reachable-history secret scanning and complete lessons.
