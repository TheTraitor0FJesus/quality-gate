...
- **Project contract** — `quality_gate/contracts.py` is the source of truth for schema 2 repository, Python/web component, verdict, and waiver models; `templates/quality-gate.toml` is the manifest shape and `templates/quality.yml` plus `templates/dependabot.yml` are the consumer CI shapes generated for a project by `$setup-repo`.
